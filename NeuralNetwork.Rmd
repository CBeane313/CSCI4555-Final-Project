...
#Drop columns that will not be useful, that are redundant, and to keep the number of features reasonable
# steamdata <- subset(steamdata, select = -c(id, game, link, store_genres, store_asset_mod_time, players_right_now, twenty4_hr_peak, all_time_peak, all_time_peak_date, publisher, release, primary_genre, developer, engine))
...
steamdata <- subset(steamdata, select = -c(id, game, link, store_genres, store_asset_mod_time, players_right_now, twenty4_hr_peak, all_time_peak, all_time_peak_date, publisher, release, primary_genre, developer, engine, peak_players))
#Show head of data
...
```{r}
linearmod <- lm(rating ~ positive_reviews*negative_reviews*total_reviews*review_percentage, data = steamdata)
...
trainx <- as.matrix(subset(train, select = -c(rating)))
...
testx <- as.matrix(subset(test, select = -c(rating)))

# R Squared in the evaluation is not calculated properly
results <- model %>% evaluate(testx, test$rating)
```
